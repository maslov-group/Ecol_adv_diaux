...
def GeneratePool(N, rho, gsigma, tau_0_dist):
...
def AssemblePool(species_list, Rs, D):
    C = EcoSystem([])
    history = []
    b_list, id_list = [], []
...
def RunTest(rho, n_community, pool_size, res_avail=R, D=D, gsigma=gsigma, tau_0_dist=tau_0_dist):
    output = {}
    params = {"gmean":gmean, "gsigma":gsigma, "gC":gC, "R":R, "D":D}
    data = []
    for ic in range(n_community):  
        data.append([])
        # make the random supply of species -- uniformly sample on symplex
        cuts = np.sort(np.random.rand(res_avail-1))
        cuts = np.insert(cuts, 0, 0)
        cuts = np.insert(cuts, res_avail, 1)
        Rs = (cuts[1:]-cuts[:-1])*res_avail
        Rs = np.pad(Rs, (0, max(0, R - len(Rs))), mode='constant') # in case of R<4
        print(Rs)
        pool0 = GeneratePool(pool_size, rho, gsigma, tau_0_dist)
        history = AssemblePool(pool0, Rs, D)
...
if __name__=="__main__":
    ...
    res_avail = parameters_list[parameters_ind]
    D1, gsigma1, tau0_dist1 = 1000, 0.1, lambda x: np.random.uniform(0.2, 0.4)
    output = RunTest(rho, n_community, pool_size, res_avail, D1, gsigma1, tau0_dist1)
    pickle.dump(output, open(f"../data/seq_vs_co_lag_new/rho={rho}_res={res_avail}_pool={pool_size}_community.pkl", "wb"))

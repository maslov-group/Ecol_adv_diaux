...
# use the alternate lag equation
...
def TaufCout(Rs, rho, tau0):
    ...
    if(nr_pres==nr):
        ...
        return tau0*( 1/(rho/nr) - (1/nr) )
    else:
        return tau0*( -(1-(nr-nr_pres-1)*rho/nr)/(nr_pres+1) + (1-(nr-nr_pres)*rho/nr)/nr_pres )
...
tau_0_dist = lambda x: np.random.uniform(0.02, 0.04)
...
if __name__=="__main__":
    ...
    pool_size = [2, 100][pool_size_ind]
    n_community = [2000, 200][pool_size_ind]
    output = RunTest(rho, n_community, pool_size)
    pickle.dump(output, open(f"../data/seq_vs_co_lag_new/rho={rho}_size={pool_size}_altlag.pkl", "wb"))

...
tau_0_dist = lambda x: np.random.uniform(0, 0)

def GeneratePool(N, rho, N_best, gsigma=gsigma, tau_0_dist=tau_0_dist):
...
def AssemblePool(species_list, Rs, D=D):
    C = EcoSystem([])
    history = []
    b_list, id_list = [], []
...
def RunTest(rho, n_community, N_best, pool_size=100):
    output = {}
    ...
    for ic in range(n_community):  
        ...
        Rs = (cuts[1:]-cuts[:-1])*R
        pool0 = GeneratePool(pool_size, rho, N_best)
...
if __name__=="__main__":
    ...
    n_rho = 40
    rho_ind = ind % n_rho
    N_best_ind = ind // n_rho
    rho = np.linspace(1e-4, 0.4, n_rho)[rho_ind]
    N_best = [1, 2, 3, 4][N_best_ind]
    n_community = 200
    output = RunTest(rho, n_community, N_best)
    pickle.dump(output, open(f"../data/seq_vs_co_lag_new/rho={rho}_N_best={N_best}_nolag.pkl", "wb"))